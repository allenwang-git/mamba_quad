...
def get_args():
  ...
  parser.add_argument('--smooth_coeff', type=int, default=5,
                      help='smooth coeff')
...
if args.tags is None:
  args.tags = args.id
assert len(args.tags) == len(args.id), "Lens: {}, {}".format(
  len(args.tags), len(args.id))
...
for eachcolor, eachlinestyle, exp_name, exp_tag in zip(colors, linestyles_choose, args.id, args.tags):
  ...
  ax1.plot(final_step, all_mean, label=exp_tag, color=eachcolor,
           linestyle=eachlinestyle, linewidth=2)
  ax1.plot(final_step, all_upper, color=eachcolor,
           linestyle=eachlinestyle, alpha=0.23, linewidth=1)
  ax1.plot(final_step, all_lower, color=eachcolor,
           linestyle=eachlinestyle, alpha=0.23, linewidth=1)
  ax1.fill_between(final_step, all_lower, all_upper,
                   alpha=0.2, color=eachcolor)
...
plt.title("{} {}".format(title_pre, args.entry), fontsize=20)
